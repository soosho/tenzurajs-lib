...
describe('tenzurajs-lib (BIP32)', function () {
  it('can import a BIP32 testnet xpriv and export to WIF', function () {
    ...
    var node = tenzura.HDNode.fromBase58(xpriv, tenzura.networks.testnet)
  ...
  it('can create a BIP32 wallet external address', function () {
    var path = "m/0'/0/0"
    var root = tenzura.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP44, tenzura, account 0, external address', function () {
  ...
  it('can recover a BIP32 parent private key from the parent public key, and a derived, non-hardened child private key', function () {
    function recoverParent (master, child) {
      ...
      for (var i = 0; i < tenzura.HDNode.HIGHEST_BIT; ++i) {
        ...
        var Qp = new tenzura.ECPair(d2).Q
        if (Qp.equals(QP)) break
      }

      var node = new tenzura.HDNode(new tenzura.ECPair(d2), master.chainCode, master.network)
    ...
    var master = tenzura.HDNode.fromSeedBuffer(seed)
...
//     var mnemonic = bip39.generateMnemonic()
    ...
    var root = tenzura.HDNode.fromSeedBuffer(seed)
