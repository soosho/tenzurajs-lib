...
function stealthSend (e, Q) {
  ...
  var c = bigi.fromBuffer(tenzura.crypto.sha256(eQ.getEncoded()))
  var cG = G.multiply(c)
  var vG = new tenzura.ECPair(null, Q.add(cG))
...
function stealthReceive (d, eG) {
  ...
  var c = bigi.fromBuffer(tenzura.crypto.sha256(eQ.getEncoded()))
  var v = new tenzura.ECPair(d.add(c).mod(n))
...
function stealthRecoverLeaked (v, e, Q) {
  var eQ = Q.multiply(e) // shared secret
  var c = bigi.fromBuffer(tenzura.crypto.sha256(eQ.getEncoded()))
  var d = new tenzura.ECPair(v.subtract(c).mod(n))
...
function stealthDualSend (e, R, Q) {
  ...
  var c = bigi.fromBuffer(tenzura.crypto.sha256(eQ.getEncoded()))
  var cG = G.multiply(c)
  var vG = new tenzura.ECPair(null, R.add(cG))
...
function stealthDualReceive (d, r, eG) {
  var eQ = eG.multiply(d) // shared secret
  var c = bigi.fromBuffer(tenzura.crypto.sha256(eQ.getEncoded()))
  var v = new tenzura.ECPair(r.add(c).mod(n))

  return v
}

describe('tenzurajs-lib (crypto)', function () {
  it('can generate a single-key stealth address', function () {
    // XXX: should be randomly generated, see next test for example
    var recipient = tenzura.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    var nonce = tenzura.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a single-key stealth address (randomly)', function () {
    var recipient = tenzura.ECPair.makeRandom() // private to recipient
    var nonce = tenzura.ECPair.makeRandom() // private to sender
  ...
  it('can generate a dual-key stealth address', function () {
    // XXX: should be randomly generated, see next test for example
    var recipient = tenzura.ECPair.fromWIF('5KYZdUEo39z3FPrtuX2QbbwGnNP5zTd7yyr2SC1j299sBCnWjss') // private to recipient
    var scan = tenzura.ECPair.fromWIF('L5DkCk3xLLoGKncqKsWQTdaPSR4V8gzc14WVghysQGkdryRudjBM') // private to scanner/recipient
    var nonce = tenzura.ECPair.fromWIF('KxVqB96pxbw1pokzQrZkQbLfVBjjHFfp2mFfEp8wuEyGenLFJhM9') // private to sender
  ...
  it('can generate a dual-key stealth address (randomly)', function () {
    var recipient = tenzura.ECPair.makeRandom() // private to recipient
    var scan = tenzura.ECPair.makeRandom() // private to scanner/recipient
    var nonce = tenzura.ECPair.makeRandom() // private to sender
